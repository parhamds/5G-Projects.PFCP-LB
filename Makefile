...
docker-build:
	for target in $(DOCKER_TARGETS); do \
		...
		if [ $(DOCKER_BUILDKIT) = 1 ]; then \
			DOCKER_CACHE_ARG="--cache-from ${DOCKER_REGISTRY}${DOCKER_REPOSITORY}upf-scaler-$$target:${DOCKER_TAG}"; \
		fi; \
		DOCKER_BUILDKIT=$(DOCKER_BUILDKIT) docker build $(DOCKER_PULL) $(DOCKER_BUILD_ARGS) \
			--target $$target \
			$$DOCKER_CACHE_ARG \
			--tag ${DOCKER_REGISTRY}${DOCKER_REPOSITORY}upf-scaler-$$target:${DOCKER_TAG} \
			--label org.opencontainers.image.source="https://github.com/parhamds/upf-scaler" \
...
docker-push:
	for target in $(DOCKER_TARGETS); do \
		docker push ${DOCKER_REGISTRY}${DOCKER_REPOSITORY}upf-scaler-$$target:${DOCKER_TAG}; \
	done
...
test: .coverage
	docker run --rm -v $(CURDIR):/upf-scaler -w /upf-scaler golang:latest \
		go test \
			-race \
			-failfast \
...
check-reuse:
	@docker run --rm -v $(CURDIR):/upf-scaler -w /upf-scaler omecproject/reuse-verify:latest reuse lint
